...
import pandas as pd
from utils import input_number, input_options, input_phase
from utils import save_exploratory_results, waveform_capture, get_actual_experiment
...
def pre_experimemnt_information(dict_results_row, Identificador_varistor=None):
    ...
    # I_fuga_pre
    I_fuga_pre = input_number('Qual a corrente de fuga pré experimento em [uA]? ')
...
def pos_experimemnt_information(dict_results_row):
    ...
    """
    # I_fuga_pos
    I_fuga_pos = input_number('Qual a corrente de fuga pós experimento em [uA]? ')
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Verifique se o osciloscópio está conectado")
    ...
    while(preceed_experiment):
        ...
        while(proceed_varistor):
            ...
            if scope.query("TRIGGER:STATE?") == 'SAVE\n':
                time.sleep(1)
                Time, Amplitude = waveform_capture(1, scope)
                time.sleep(2)
                ...
                # plt.show()
                _, Amplitude = waveform_capture(2, scope)
                time.sleep(2)
                Ensaio['I_surto_medida[uA]'] = Amplitude * 40
                Ensaio['Power[W]'] = Ensaio['V_grampeamento[V]']*Ensaio['I_surto_medida[uA]']
